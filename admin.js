...
function readFileAsDataUrl(file, maxSide = 1200, quality = .82) {
...
$('saveButton').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!res.ok || !result.ok) throw new Error(result.message || 'Kaydedilemedi');
    state = payload;
    message.textContent = 'Kaydedildi. Siteyi açınca güncel halini göreceksin.';
  } catch (error) {
    message.textContent = 'Netlify Functions çalışmıyorsa sadece bu tarayıcıda önizleme kaydedildi. Hata: ' + error.message;
  }
});
