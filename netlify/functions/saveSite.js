...
exports.handler = async (event) => {
  ...
  if (!event.body || event.body.length > MAX_BODY_SIZE) {
    return {
      statusCode: 413,
      headers: { 'Content-Type': 'application/json; charset=utf-8' },
      body: JSON.stringify({ ok: false, message: 'Dosya çok büyük. Fotoğrafları biraz küçültüp tekrar dene.' })
    };
  }
  ...
  try {
    ...
    return {
      statusCode: 400,
      headers: { 'Content-Type': 'application/json; charset=utf-8' },
      body: JSON.stringify({ ok: false, message: 'Kaydedilemedi.', detail: error.message })
    };
  }
};
